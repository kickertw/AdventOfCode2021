from utils import *
from functools import reduce

def performOperation(typeId, level, allVals):
  global debug
  spaces = '  ' * level
  retVal = None
  
  if typeId == 0:
    retVal = sum(allVals)
    debugPrint(debug, f'{spaces} >> Sum of {allVals} = {retVal}')
  elif typeId == 1:
    retVal = reduce((lambda a, b: a*b), allVals)
    debugPrint(debug, f'{spaces} >> Product of {allVals} = {retVal}')
  elif typeId == 2:
    retVal = min(allVals)
    debugPrint(debug, f'{spaces} >> Min of {allVals} = {retVal}')
  elif typeId == 3:
    retVal = max(allVals)
    debugPrint(debug, f'{spaces} >> Max of {allVals} = {retVal}')
  elif typeId == 5:
    retVal = 1 if allVals[0] > allVals[1] else 0
    debugPrint(debug, f'{spaces} >> {allVals[0]} > {allVals[1]} = {retVal}')
  elif typeId == 6:
    retVal = 1 if allVals[0] < allVals[1] else 0
    debugPrint(debug, f'{spaces} >> {allVals[0]} < {allVals[1]} = {retVal}')
  elif typeId == 7:
    retVal = 1 if allVals[0] == allVals[1] else 0
    debugPrint(debug, f'{spaces} >> {allVals[0]} == {allVals[1]} = {retVal}')

  return retVal

def parseLiteral(version, input, level):
  #global packetValues
  global debug
  spaces = '  ' * level

  literalBinaryVal = ''
  isLastGroup = False
  subPacketCount = 0
  while True:
    isLastGroup = True if input[0] == '0' else False
    literalBinaryVal += input[1:5]
    input = input[5:]
    subPacketCount += 1
    if isLastGroup:
      break
  
  decVal = convertBinToDec(literalBinaryVal)
  debugPrint(debug, f'{spaces} Parsing Literal [V = {version} / # of Subpackets = {subPacketCount} / {literalBinaryVal} = {decVal}]')
  # packetValues[version].append(decVal)
  
  return input, decVal

def parseOperator(input, typeId, level):
  global debug

  allVals = []
  spaces = '  ' * level
  lTypeId = input[0]
  lbits = 15 if lTypeId == '0' else 11

  input = input[1:]
  lengthVal = convertBinToDec(input[:lbits])
  lengthTypeDebug = 'packets' if lTypeId == '1' else 'bits'
  debugPrint(debug, f'{spaces} I = {lTypeId} / L = has {lengthVal} {lengthTypeDebug}')
  if lTypeId == '0':
    #we know the number of bits to look at
    input1 = input[lbits:lbits+lengthVal]
    while len(input1) > 0 and int(input1) > 0:
      input1, decVal = parsePacket(input1, level+1)
      allVals.append(decVal)

    input = input[lbits+lengthVal:]
  else:
    #we know the number of packets
    input = input[lbits:]
    for _ in range(lengthVal):
      input, decVal = parsePacket(input, level+1)
      allVals.append(decVal)
  
  retVal = performOperation(typeId, level, allVals)
  return input, retVal

def parsePacket(input, level = 0):
  global versionSum
  global debug
  spaces = '  ' * level
  decVal = None

  if len(input) == 0 or int(input) == 0:
    return ''

  version = convertBinToDec(input[:3])
  typeId = convertBinToDec(input[3:6])
  versionSum += version
  # If literal
  if typeId == 4:    
    debugPrint(debug, f'{spaces} Literal Found! Version={version}')
    input, decVal = parseLiteral(version, input[6:], level)
  else:
    input = input[6:]
    debugPrint(debug, f'{spaces} Operator Found! Version={version}')
    input, decVal = parseOperator(input, typeId, level)
  return input, decVal

versionSum = 0
debug = True
#packetValues = defaultdict(list)
binVal = convertHexToBin('')
_, p2Answer = parsePacket(binVal)

print(f'Version Sum = {versionSum}')
print(f'P2 = {p2Answer}')